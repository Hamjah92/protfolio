...
const Work = ({ data: works }: IWorkProps) => {
  const { filterWork, handleWorkFilter, animateCardVariants, controls } =
    FilterWorks(works);
    ...
    <SectionLayout idName={"work"} classNames={"bg-primary"}>
      ...
      >
        ...
        <motion.div
          ...
          animate={controls}
          className={
            "grid grid-flow-col-dens grid-cols-1 gap-8 lg:grid-cols-2 xl:grid-cols-3 place-items-center"
          }
        >
          {filterWork &&
            filterWork.map((work) => (
