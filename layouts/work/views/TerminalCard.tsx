...
const TerminalCard = ({ work }: IProps) => {
  ...
  return (
    <motion.div
      ...
      className={
        "w-[370px] min-h-[400px] rounded-xl text-black text-xs shadow-sm bg-clip-padding backdrop-blur-lg bg-gradient-to-b from-gray-300/80 to-gray-300/40 transition-all duration-[0.4s] ease-in-out hover:shadow-2xl"
      }
    >
        ...
        <code>
          <div className={"px-2 pb-8 pt-4 w-full h-full space-y-4"}>
            ...
            <div className={"space-y-2"}>
              ...
              <RenderIf when={work.tags.includes("credentials")}>
                <>
                  <p>{"$"} Login</p>
                  <p>UserId : Demo@demo.com</p>
                  <p>Password : 123456</p>
                </>
              </RenderIf>
            </div>
...
const Banner = ({
  ...
  <div className={"flex items-center shadow-md p-2"}>
    ...
    <code className={"flex-1 flex justify-center font-medium text-[0.68rem]"}>
      {`root@project:~/${activeTab}/${projectTitle}`}
    </code>
...
const ImageAndOverly = ({
  ...
  <div className={"relative w-full h-48"}>
    ...
    <motion.div
      ...
      className={
        "absolute left-0 right-0 bg-black/50 w-full h-48 flex items-center justify-center space-x-10 text-gray-500 "
      }
    >
      {!!codeLink && (
        <motion.a
          ...
          href={codeLink}
          className={
            "rounded-full p-2 w-11 h-autos bg-black/90 transition-all duration-200 ease-in-out hover:text-white"
          }
        >
          <CodeIcon />
        </motion.a>
